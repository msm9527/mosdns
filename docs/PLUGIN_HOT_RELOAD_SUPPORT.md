...
本文档说明当前仓库里哪些插件支持 `ControlConfigReloader` 热重载，哪些不支持，以及这些插件对应的配置文件位置。
...
当前代码里，插件是否支持“配置热重载”，统一以是否实现 `coremain.ControlConfigReloader` 为准：
...
- 支持 `ControlConfigReloader` 热重载的插件类型：11 个
...
下表这些插件在 `config/config.yaml` 的默认加载链路里会被创建，但它们本身不实现 `ControlConfigReloader`。如果直接改它们的 YAML 定义，通常仍然需要重启进程才能完整生效。

| 插件类型 | 源码文件 | 对应配置文件 | 备注 |
| --- | --- | --- | --- |
| `cache` | `plugin/executable/cache/cache.go` | `config/sub_config/21-data-cache-upstreams.yaml` `config/sub_config/cache_policies.yaml` | 缓存清空可即时执行，但缓存插件配置本身不支持热重载 |
...
下列插件在源码里已注册，但默认 `config/config.yaml` 并不会加载它们。它们当前也没有实现 `ControlConfigReloader`。
...
1. 插件实现 `coremain.ControlConfigReloader`
2. 本文档的支持矩阵
