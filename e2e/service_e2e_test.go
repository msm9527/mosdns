...
func (fx *serviceE2EFixture) testRoutingModeSwitches(t *testing.T, rec *e2eCaseRecorder) {
	...
	fx.setSwitch(t, "cn_answer_mode", "realip")
	subscriptionCNResp := fx.queryUDP(t, "cn.example", dns.TypeA)
	requireServiceE2EARecord(t, subscriptionCNResp, "1.1.1.1")
	cnRealipResp := fx.queryUDP(t, "whitelist-cn.example", dns.TypeA)
	requireServiceE2EARecord(t, cnRealipResp, "1.1.1.1")
	fx.setSwitch(t, "cn_answer_mode", "fakeip")
	subscriptionCNAftFakeModeResp := fx.queryUDP(t, "cn.example", dns.TypeA)
	requireServiceE2EARecord(t, subscriptionCNAftFakeModeResp, "1.1.1.1")
	cnFakeResp := fx.queryUDP(t, "whitelist-cn.example", dns.TypeA)
	...
	proxyWhitelistLoopbackResp := fx.queryUDP(t, "proxy.example", dns.TypeA)
	requireServiceE2EARecord(t, proxyWhitelistLoopbackResp, "28.0.0.2")
	fx.setSwitch(t, "client_proxy_mode", "all")
	rec.SetDetail("routing switches changed whitelist domestic realip/fakeip while subscription-direct CN stayed realip; loopback clients bypass client whitelist gating and keep proxy path")
	recordDNSCheck(rec, "subscription cn realip", "udp", fx.dnsAddr, "cn.example", dns.TypeA, subscriptionCNResp)
	recordDNSCheck(rec, "subscription cn fake mode realip", "udp", fx.dnsAddr, "cn.example", dns.TypeA, subscriptionCNAftFakeModeResp)
	recordDNSCheck(rec, "whitelist cn realip", "udp", fx.dnsAddr, "whitelist-cn.example", dns.TypeA, cnRealipResp)
	recordDNSCheck(rec, "whitelist cn fakeip", "udp", fx.dnsAddr, "whitelist-cn.example", dns.TypeA, cnFakeResp)
	recordDNSCheck(rec, "proxy all", "udp", fx.dnsAddr, "proxy.example", dns.TypeA, proxyAllResp)
	recordDNSCheck(rec, "proxy whitelist loopback", "udp", fx.dnsAddr, "proxy.example", dns.TypeA, proxyWhitelistLoopbackResp)
}
...
func (fx *serviceE2EFixture) testCacheAndStability(t *testing.T, rec *e2eCaseRecorder) {
	...
	requireServiceE2EARecord(t, cnFakeFirst, "1.1.1.1")
	requireServiceE2EARecord(t, cnFakeSecond, "1.1.1.1")
	cnSwitchFakeFirst := fx.queryUDP(t, "whitelist-cn.example", dns.TypeA)
	cnSwitchFakeSecond := fx.queryUDP(t, "whitelist-cn.example", dns.TypeA)
	requireServiceE2EARecord(t, cnSwitchFakeFirst, "30.0.0.2")
	requireServiceE2EARecord(t, cnSwitchFakeSecond, "30.0.0.2")
	proxyFirst := fx.queryUDP(t, "proxy-cache.example", dns.TypeA)
	proxyAnswer := requireServiceE2EAnyARecord(t, proxyFirst).A.String()
	proxySecond := fx.queryUDP(t, "proxy-cache.example", dns.TypeA)
	requireServiceE2EARecord(t, proxySecond, proxyAnswer)
	probeCacheFirst := fx.queryProbe(t, "probe-cache.example", dns.TypeAAAA)
	probeCacheSecond := fx.queryProbe(t, "probe-cache.example", dns.TypeAAAA)
	requireServiceE2EAAAARecord(t, probeCacheFirst, "2001:db8::8")
	requireServiceE2EAAAARecord(t, probeCacheSecond, "2001:db8::8")
	probeEntries := fx.cacheEntries(t, "cache_probe", "probe-cache.example")
	...
	recordDNSCheck(rec, "subscription cn realip first", "udp", fx.dnsAddr, "cn.example", dns.TypeA, cnFakeFirst)
	recordDNSCheck(rec, "subscription cn realip second", "udp", fx.dnsAddr, "cn.example", dns.TypeA, cnFakeSecond)
	recordDNSCheck(rec, "whitelist cn fakeip first", "udp", fx.dnsAddr, "whitelist-cn.example", dns.TypeA, cnSwitchFakeFirst)
	recordDNSCheck(rec, "whitelist cn fakeip second", "udp", fx.dnsAddr, "whitelist-cn.example", dns.TypeA, cnSwitchFakeSecond)
	recordDNSCheck(rec, "proxy cache first", "udp", fx.dnsAddr, "proxy-cache.example", dns.TypeA, proxyFirst)
	recordDNSCheck(rec, "proxy cache second", "udp", fx.dnsAddr, "proxy-cache.example", dns.TypeA, proxySecond)
	recordDNSCheck(rec, "probe cache first", "udp", fx.sbnodeAddr, "probe-cache.example", dns.TypeAAAA, probeCacheFirst)
	recordDNSCheck(rec, "probe cache second", "udp", fx.sbnodeAddr, "probe-cache.example", dns.TypeAAAA, probeCacheSecond)
	rec.AddCheck("cache_probe entries", fmt.Sprintf("%+v", probeEntries.Items))
...
func requireServiceE2ECacheHits(t *testing.T, item coremain.CacheStatsSnapshot) {
	...
	if item.Counters["query_total"] < 2 || item.Counters["hit_total"] < 1 {
		t.Fatalf("unexpected cache stats for %s: backend=%d l1=%d counters=%+v config=%+v", item.Tag, item.BackendSize, item.L1Size, item.Counters, item.Config)
	}
}
...
func (fx *serviceE2EFixture) testCacheClientTTLGovernance(t *testing.T, rec *e2eCaseRecorder) {
	...
	cnFakeFirst := fx.queryUDP(t, "ttl-cn-switch.example", dns.TypeA)
	cnFakeSecond := fx.queryUDP(t, "ttl-cn-switch.example", dns.TypeA)
	...
	recordDNSCheck(rec, "cn fakeip ttl governed first", "udp", fx.dnsAddr, "ttl-cn-switch.example", dns.TypeA, cnFakeFirst)
	recordDNSCheck(rec, "cn fakeip ttl governed second", "udp", fx.dnsAddr, "ttl-cn-switch.example", dns.TypeA, cnFakeSecond)
