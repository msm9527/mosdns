...
func decideShuntAction(qtype string, marks map[uint8]bool, switches map[string]string, rules []shuntRuleConfig) (shuntDecision, []shuntDecisionStep) {
	...
	for _, step := range []struct {
		...
		appendStep("sequence_known_domain", step.Mark, action, reason, matched, matched)
		if matched {
			return shuntDecision{Stage: "sequence_known_domain", Action: action, Reason: reason, Matched: step.Mark}, path
		}
	}
