...
func (a *shuntAnalyzer) Explain(domainName, qtype string) (*shuntExplainResult, error) {
	...
	result := &shuntExplainResult{
		...
		Matches:     matches,
		Decision:    decideShuntAction(qtype, markSet, a.switches),
...
func decideShuntAction(qtype string, marks map[uint8]bool, switches map[string]string) shuntDecision {
	...
	case "SOA", "PTR", "HTTPS", "TYPE65":
		if blockQueryType {
			return shuntDecision{Stage: "precheck", Action: "reject", Reason: "block_query_type:on for special qtype"}
		}
	case "AAAA":
		if blockIPv6 {
			return shuntDecision{Stage: "precheck", Action: "reject", Reason: "block_ipv6:on", Matched: 28}
		}
	}

	if marks[1] && blockResponse {
		return shuntDecision{Stage: "precheck", Action: "reject", Reason: "blocklist + block_response:on", Matched: 1}
	}
	if qtype == "A" && marks[2] && blockResponse {
		return shuntDecision{Stage: "precheck", Action: "reject", Reason: "记忆无V4 + block_response:on", Matched: 2}
	}
	if qtype == "AAAA" && marks[3] && blockResponse {
		return shuntDecision{Stage: "precheck", Action: "reject", Reason: "记忆无V6 + block_response:on", Matched: 3}
	}
	if marks[5] && adBlock {
		return shuntDecision{Stage: "precheck", Action: "reject", Reason: "广告屏蔽 + ad_block:on", Matched: 5}
	}
	if marks[6] {
		return shuntDecision{Stage: "precheck", Action: "domestic", Reason: "DDNS 域名直接走国内上游", Matched: 6}
	}
	...
	for _, step := range []struct {
		...
		if marks[step.Mark] {
			return shuntDecision{Stage: "sequence_known_domain", Action: step.Action, Reason: step.Reason, Matched: step.Mark}
		}
	}
