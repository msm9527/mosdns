...
func saveGlobalOverridesToCustomConfig(payload *GlobalOverrides) error {
	...
	buf.WriteString("# - socks5: 给支持 socks5 的插件提供统一代理地址，例如 127.0.0.1:7891\n")
...
func saveSwitchesToCustomConfig(values map[string]string) error {
	...
	buf.WriteString("# - branch_cache: on/off，控制国内/国外分支缓存。\n")
	buf.WriteString("# - main_cache: on/off，控制主缓存总开关。\n")
