...
func (f *AliAPI) configureStatsPersistence(controlDBPath string) error {
...
func (f *AliAPI) bindStatsCallbacks(us []*upstreamWrapper) {
	for _, u := range us {
		if u != nil {
...
func (f *AliAPI) restorePersistentStats(us []*upstreamWrapper) error {
...
func (f *AliAPI) flushPersistentStats() error {
	if f.controlDBPath == "" || f.pluginTag == "" {
		return nil
	}
...
func (f *AliAPI) snapshotPersistentStats() []coremain.UpstreamRuntimeStats {
...
func (f *AliAPI) markStatsDirty() {
	if f != nil && f.statsFlusher != nil {
		f.statsFlusher.MarkDirty()
	}
}

func (f *AliAPI) closeStatsFlusher() error {
	if f == nil || f.statsFlusher == nil {
		return nil
	}
...
func (f *AliAPI) ResetUpstreamStats(_ context.Context, upstreamTag string) (int, error) {
	_, us := f.snapshotRuntime()
	count := 0
	for _, u := range us {
		if u == nil {
