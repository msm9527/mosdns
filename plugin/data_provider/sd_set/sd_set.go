...
func (p *SdSet) GetRulesShared() ([]string, error) {
	p.mu.RLock()
	defer p.mu.RUnlock()
	return p.rules, nil
}
...
func (p *SdSet) reloadAllRules(options coremain.RuleSourceSyncOptions) error {
...
func (p *SdSet) currentSyncState() []coremain.RuleSourceVersion {
	p.mu.RLock()
	defer p.mu.RUnlock()
